	...
	try{
		...
		if(data.result == null && data[0].result== 'success'){
			for(let i = 1; i <data.length ; i++){
				...
				let man_img = "/img/staff_man.jpg";
				let woman_img = "/img/staff_woman.jpg"
	...
	$('.body_text').fadeOut(100,function(){
		switch(currenStep){
			...
			case 1:
				...
				changeCss("new_css/appointment.css");
				step1_js();
				break;
			case 2:
				$(".body_text").html(serviceSelection);
				changeCss("new_css/serviceSelection.css");
				step2_js();
				break;
			case 3:
				$(".body_text").html(petInformation)
				changeCss("new_css/pet_information.css");
				step3_js();
				break;
			case 4:
				$('.body_text').html(agreement)
				changeCss("new_css/agreement.css")
				break;
			case 5:
				$('.body_text').html(confirmation)
				changeCss("new_css/confirmation.css")
				step5_js()
				break;
			}
		changePage();
