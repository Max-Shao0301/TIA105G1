...
public interface OrdersRepository extends JpaRepository<OrdersVO, Integer> {
	...
	@Query(value = "SELECT COUNT(*) FROM orders WHERE mem_id = ?1 AND status != 3 ", nativeQuery = true)
	...
	List<OrdersVO> findOrdersByMemIdWithPagination(@Param("memId") Integer memId, @Param("offset") int offset,
			@Param("limit") int limit);

	@EntityGraph(attributePaths = { "schedule", "staff", "pet", "pet.pet" })
	@Query("SELECT DISTINCT o FROM OrdersVO o " + "JOIN o.schedule s " + "JOIN o.staff staff " + "JOIN o.pet op "
			+ "JOIN op.pet pet " + "WHERE o.member.memId = :memId AND (" + "o.onLocation LIKE %:keyword% "
			+ "OR o.offLocation LIKE %:keyword% " + "OR staff.staffName LIKE %:keyword% "
			+ "OR staff.carNumber LIKE %:keyword% " + "OR pet.petName LIKE %:keyword%)")
	...
	@Query(value = "SELECT"
			...
			+ " p.type, p.pet_name, p.pet_gender, p.weight," + " s.timeslot, s.date," + " m.mem_phone "
			+ "FROM orders o " + "JOIN order_pet op ON o.order_id = op.order_id "
			+ "JOIN pet p ON op.pet_id = p.pet_id " + "JOIN schedule s ON o.sch_id = s.sch_id "
